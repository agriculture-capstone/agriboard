<template>
  <div class='Loans'>
    <md-table class="table" v-model="loans" md-sort="name" md-sort-order="asc" @md-selected="onSelect">
      <md-table-toolbar>
        <div class="md-toolbar-section-start">
          <h1 class="md-title"><md-icon class="md-size-2x icon">receipt</md-icon>Loans</h1>
        </div>
      ...
      <md-table-row slot="md-table-row" slot-scope="{ item }" md-selectable="single" md-auto-select>
        ...
        <md-table-cell md-label="From" md-sort-by="from">{{item.fromPersonName}}</md-table-cell>
        <md-table-cell md-label="To" md-sort-by="to">{{item.toPersonName}}</md-table-cell>
        <md-table-cell :md-label="`Value (${item.currency})`" md-sort-by="costPerUnit">{{item.amount}}</md-table-cell>
      </md-table-row>
    </md-table>

    <div class="view-dialog-wrapper">
      <md-dialog v-if="selectedRow" :md-active.sync="showViewDialog">
        <md-dialog-title>Loan Details</md-dialog-title>
          <md-dialog-content>
            ...
            <h3><b>From</b></h3>
            {{ selectedRow.fromPersonName }}
            <h3><b>To</b></h3>
            {{ selectedRow.toPersonName }}
...
export default Vue.extend({
  ...
  computed: {
    loans (): any {
      return this.$store.state.loan.rows.map((row: any) => {
        return {
          ...row, 
